...
alias sm='nohup /Applications/Sublime\ Merge.app/Contents/MacOS/sublime_merge "${1:-.}" 2>&1 /dev/null &'
alias smerge="nohup /Applications/Sublime\ Merge.app/Contents/MacOS/sublime_merge 2>&1 /dev/null &"
alias smerge='nohup /Applications/Sublime\ Merge.app/Contents/MacOS/sublime_merge "${1:-.}" 2>&1 /dev/null &'
alias ss="lsof -iTCP -sTCP:LISTEN -P ; netstat -an | grep -E 'Proto|LISTEN'"
alias st='nohup /Applications/Sublime\ Text.app/Contents/MacOS/sublime_text "${1:-.}" 2>&1 /dev/null &'
...
function update() {
	...
	if [ $num_packages -gt 0 ]; then
		...
		printf "\n${BOLD}→ Install brew updates ? ${NC} ${MAGENTA}(y/N)${NC}"
			brew upgrade ; brew upgrade --cask --greedy ; brew cleanup --prune= 14 ; printf "\nBrew updates ${LGREEN}done !${NC}\n"
