...
if util_binary_exists ifconfig; then
...
if util_binary_exists apachectl; then
...
util_binary_exists curl && ix() { curl -n -F 'f:1=<-' http://ix.io; }
# Used for piping to clipboard
util_binary_exists xclip && alias xcopy="xclip -se c"
