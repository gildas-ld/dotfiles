...
for daemon in "${daemons[@]}"; do
	...
	daemon_flag=$(/usr/libexec/PlistBuddy -c "Print Enabled" /System/Library/LaunchDaemons/${daemon}.plist 2>/dev/null)
	if [[ ${daemon_flag} == 'false' ]]; then
		printf "[${GREEN}SUCCESS${NC}] Enabled ${daemon}\n"
	else
		printf "[${RED}ERROR${NC}] Failed to enable ${daemon}\n"
	fi
done
...
for agent in "${agents[@]}"; do
	{
		/usr/libexec/PlistBuddy -c "Add Disabled bool false" /System/Library/LaunchAgents/${agent}.plist
		launchctl load -w /System/Library/LaunchAgents/${agent}.plist
	} &>/dev/null
	# If the disabled flag is set, print success message
	agent_flag=$(/usr/libexec/PlistBuddy -c "Print Enabled" /System/Library/LaunchAgents/${agent}.plist 2>/dev/null)
	if [[ ${agent_flag} == 'false' ]]; then
		printf "[${GREEN}SUCCESS${NC}] Enabled ${agent}\n"
	else
		printf "[${RED}ERROR${NC}] Failed to enabled ${agent}\n"
	fi
done
